	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'houses' => array('Houses', '', 'resources/table_icons/building_add.png', 'None'),
			'tenants' => array('Tenants', '', 'resources/table_icons/group_add.png', 'None'),
			'invoices' => array('Invoices', '', 'resources/table_icons/card_money.png', 'None'),
			'payments' => array('Payments', '', 'resources/table_icons/account_balances.png', 'None')
		);
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'houses' => "`houses`.`id` as 'id', `houses`.`house_number` as 'house_number', `houses`.`features` as 'features', `houses`.`rent` as 'rent', `houses`.`status` as 'status'",
			'tenants' => "`tenants`.`id` as 'id', `tenants`.`fullname` as 'fullname', `tenants`.`gender` as 'gender', `tenants`.`national_id` as 'national_id', `tenants`.`phone_number` as 'phone_number', `tenants`.`email` as 'email', if(`tenants`.`registration_date`,date_format(`tenants`.`registration_date`,'%m/%d/%Y'),'') as 'registration_date', IF(    CHAR_LENGTH(`houses1`.`house_number`), CONCAT_WS('',   `houses1`.`house_number`), '') as 'house', `tenants`.`agreement_document` as 'agreement_document', `tenants`.`status` as 'status', if(`tenants`.`exit_date`,date_format(`tenants`.`exit_date`,'%m/%d/%Y'),'') as 'exit_date'",
			'invoices' => "`invoices`.`id` as 'id', IF(    CHAR_LENGTH(`tenants1`.`fullname`) || CHAR_LENGTH(`tenants1`.`national_id`), CONCAT_WS('',   `tenants1`.`fullname`, ' ID: ', `tenants1`.`national_id`), '') as 'tenant', IF(    CHAR_LENGTH(`tenants1`.`phone_number`), CONCAT_WS('',   `tenants1`.`phone_number`), '') as 'phone', IF(    CHAR_LENGTH(`houses1`.`house_number`), CONCAT_WS('',   `houses1`.`house_number`), '') as 'house', `invoices`.`year` as 'year', `invoices`.`month` as 'month', `invoices`.`particulars` as 'particulars', `invoices`.`total` as 'total', `invoices`.`comments` as 'comments'",
			'payments' => "`payments`.`id` as 'id', IF(    CHAR_LENGTH(`tenants1`.`fullname`) || CHAR_LENGTH(`tenants1`.`national_id`) || CHAR_LENGTH(`invoices1`.`id`), CONCAT_WS('',   `tenants1`.`fullname`, ' ID: ', `tenants1`.`national_id`, ' INVID: ', `invoices1`.`id`), '') as 'tenant', IF(    CHAR_LENGTH(`houses1`.`house_number`), CONCAT_WS('',   `houses1`.`house_number`), '') as 'house', IF(    CHAR_LENGTH(`invoices1`.`year`), CONCAT_WS('',   `invoices1`.`year`), '') as 'year', IF(    CHAR_LENGTH(`invoices1`.`month`), CONCAT_WS('',   `invoices1`.`month`), '') as 'month', IF(    CHAR_LENGTH(`invoices1`.`total`), CONCAT_WS('',   `invoices1`.`total`), '') as 'expected_amount', `payments`.`paid_amount` as 'paid_amount', `payments`.`balance` as 'balance', if(`payments`.`date`,date_format(`payments`.`date`,'%m/%d/%Y'),'') as 'date', `payments`.`comments` as 'comments'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			'houses' => "`houses` ",
			'tenants' => "`tenants` LEFT JOIN `houses` as houses1 ON `houses1`.`id`=`tenants`.`house` ",
			'invoices' => "`invoices` LEFT JOIN `tenants` as tenants1 ON `tenants1`.`id`=`invoices`.`tenant` LEFT JOIN `houses` as houses1 ON `houses1`.`id`=`tenants1`.`house` ",
			'payments' => "`payments` LEFT JOIN `invoices` as invoices1 ON `invoices1`.`id`=`payments`.`tenant` LEFT JOIN `tenants` as tenants1 ON `tenants1`.`id`=`invoices1`.`tenant` LEFT JOIN `houses` as houses1 ON `houses1`.`id`=`tenants1`.`house` "
		);
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('RENTAL_MANAGEMENT_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('RENTAL_MANAGEMENT_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['RENTAL_MANAGEMENT_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['RENTAL_MANAGEMENT_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> RENTWALE</a>
			</div>
